...
└── src/uniclaw/            # 📦 包根目录
    ...
    ├── jev_compact.py      # Jev 智能压缩(noul 评分工具配对,差异化阈值,失败回退 LLM 摘要)
    ├── spinner.py          # 加载动画指示器
    │
...
- `yes_no` - 是/否判断
- `rate` - 量表评级
- `multi_judge` - 多维度批量判断(一次请求并行评估多个不同维度的问题)
...
- **上下文压缩**: 按工具配对评分决定保留/删除,区分可再生与不可再生结果差异化阈值,减少有损摘要
...
**Jev 智能压缩**(Level 1):使用 TypeSafe AI 的 noul 评分批量评估每个工具调用+结果配对,按配对决策三种处置 — 完整保留 / 仅保留调用(结果降为一行占位)/ 整对删除。文本消息(user/assistant)永远原样保留,实现"智能删除"而非"有损总结"。需配置 `TYPESAFE_API_KEY` 环境变量;Jev 不可用或调用失败时自动回退 LLM 摘要,自动压缩与手动 `/compact` 走同一条链路(`smart_compact()`)。

阈值按误留/误删代价的不对称性差异化设定 — 工具调用一行约 30 token,留错只是浪费,删错是叙事永久断裂,因此删除是例外;工具结果可达数千 token,stub 后能否重跑取回决定代价:

| 决策 | 阈值(noul) | 说明 |
|------|------------|------|
| 保留调用 | ≥ 0.3 | 低于此值才整对删除,仅限判官明确视为可抹除的探索性调用 |
| 保留结果(可再生) | ≥ 0.7 | Read/Grep/Glob/webFetch 等可重跑取回的结果,允许偏激进 |
| 保留结果(不可再生) | ≥ 0.5 | 多媒体块(图片/音频/视频)与不可重跑工具(Edit 等副作用回执)的结果,stub 即永久丢失,不确定时优先保全文 |

配套机制:多媒体结果会被单独识别并提示判官"通常无法重新获取";支持 `focus` 聚焦主题参数,优先保留与主题相关的内容;改写通过复制消息完成,不破坏 history 归档(recall 检索/会话存档依赖原文);压缩后返回 `tokens_saved` 统计实际释放量。
...
- 上下文压缩: 按工具配对价值评分保留/删除,区分可再生/不可再生结果差异化阈值,减少有损摘要
